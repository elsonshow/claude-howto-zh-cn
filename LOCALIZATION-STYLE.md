...
这份规范用于约束 `claude-howto-zh-cn` 的中文对照内容，尤其是 `zh/` 目录里的本地化文档，确保两件事同时成立：

- 中文用户更容易读懂、学会、照着做
- 文档里的命令、配置、模板、frontmatter、JSON/YAML 片段仍然可运行

当前仓库采用“英文根目录 + `zh/` 中文对照目录”的结构。根目录优先跟随上游英文主线；中文改写、补充说明和本土化表达应放在 `zh/` 对应路径里，避免根目录出现中英文混排。

## 核心原则

### 0. 英文根目录保持上游一致

根目录文档、脚本、CI、资源文件默认按 `upstream/main` 同步。除非是 fork 维护说明文件，例如 `UPSTREAM.md` 和 `LOCALIZATION-STYLE.md`，否则不要在根目录英文文档里追加中文段落。
...
- 如果英文根目录已有对应文件，中文文档应尽量保持相同路径，例如 `README.md` 对应 `zh/README.md`，`03-skills/README.md` 对应 `zh/03-skills/README.md`。
